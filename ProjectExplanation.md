...
- `public/index.html`: Minimal HTML structure with textarea and preview.
- `public/editor.js`: Core editor logic (Socket.IO events, Markdown rendering).
- `public/style.css`: Minimal CSS for layout and styling.
...
- Two-column layout:
  ...
  - Right: Markdown preview using `marked`.
- Minimal styling with essential functionality only.
...
- Start: `node server.js` (serves at http://localhost:5000)

### Frontend:
- Static files served by Express, no separate build step needed.

## 9) Error handling and caveats
...
- **Frontend**: Vanilla JavaScript, marked, CSS.
